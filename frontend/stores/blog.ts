  ...
  actions: {
    async fetchPosts(includeDrafts = false) {
      ...
      try {
        ...
        const response = await fetch(`${config.public.apiBase}/blog/posts${params}`)
    ...
    async fetchPostBySlug(slug: string) {
      ...
      try {
        const config = useRuntimeConfig()
        const response = await fetch(`${config.public.apiBase}/blog/posts/${slug}`)
    ...
    async fetchPostsByCategory(category: string, includeDrafts = false) {
      ...
      try {
        const config = useRuntimeConfig()
        const params = includeDrafts ? '?drafts=true' : ''
        const response = await fetch(`${config.public.apiBase}/blog/category/${category}${params}`)
    ...
    async fetchPostsByTag(tag: string, includeDrafts = false) {
      ...
      try {
        const config = useRuntimeConfig()
        const params = includeDrafts ? '?drafts=true' : ''
        const response = await fetch(`${config.public.apiBase}/blog/tag/${tag}${params}`)
